...
export async function PUT(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (
      updateData.percentSubsidy === undefined &&
    ...
    const feeAmountFieldsUpdated =
      (updateData.tuitionFee !== undefined &&
        Number(updateData.tuitionFee) !== Number(existingScholarship.tuitionFee || 0)) ||
      (updateData.miscellaneousFee !== undefined &&
        Number(updateData.miscellaneousFee) !==
          Number(existingScholarship.miscellaneousFee || 0)) ||
      (updateData.laboratoryFee !== undefined &&
        Number(updateData.laboratoryFee) !== Number(existingScholarship.laboratoryFee || 0)) ||
      (updateData.otherFee !== undefined &&
        Number(updateData.otherFee) !== Number(existingScholarship.otherFee || 0));

    const subsidyFieldsUpdated =
      (updateData.amountSubsidy !== undefined &&
        Number(updateData.amountSubsidy) !== Number(existingScholarship.amountSubsidy || 0)) ||
          ...
          await Promise.all(
            studentScholarships.map(async (ss) => {
              ...
              if (existingFees.length > 0) {
                ...
                const totalFeesForPercent = feeAmountFieldsUpdated
                ...
                // Calculate percentSubsidy as decimal (e.g., 0.1667 for 16.67%)
                const percentSubsidy =
                  totalFeesForPercent > 0
                ...
                if (feeAmountFieldsUpdated) {
                  ...
                  studentFeeUpdateData.miscellaneousFee = Number(
                    updatedScholarship.miscellaneousFee || 0
                  );
                  studentFeeUpdateData.laboratoryFee = Number(
                    updatedScholarship.laboratoryFee || 0
                  );
