...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (
      !session ||
      (session.role !== 'ADMIN' && session.role !== 'STAFF' && session.role !== 'VIEWER')
    ) {
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const actualGrantAmount =
      actualGrantType === 'TUITION_ONLY' || actualGrantType === 'NONE'
        ? 0
    ...
    return NextResponse.json({
      ...
      message: shouldCreateFees
        ? 'Scholarship assigned successfully and fees created'
