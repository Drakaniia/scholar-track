  ...
  try {
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      if (body.middleInitial !== undefined)
        updateData.middleInitial = body.middleInitial ? body.middleInitial.toUpperCase() : null;
...
async function updateStudentFees(
  ...
  const resolvedTuitionFee =
    feeData.tuitionFee ?? (existingFees ? Number(existingFees.tuitionFee) : 0);
  const resolvedOtherFee = feeData.otherFee ?? (existingFees ? Number(existingFees.otherFee) : 0);
  const resolvedMiscellaneousFee =
    feeData.miscellaneousFee ?? (existingFees ? Number(existingFees.miscellaneousFee) : 0);
  const resolvedLaboratoryFee =
    feeData.laboratoryFee ?? (existingFees ? Number(existingFees.laboratoryFee) : 0);

  const totalFees =
    resolvedTuitionFee + resolvedOtherFee + resolvedMiscellaneousFee + resolvedLaboratoryFee;
