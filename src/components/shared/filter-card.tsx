        ...
        <div className="border-b border-border/60 bg-muted/80 px-4 py-3">
          <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
            <div className="flex min-w-0 items-center gap-3">
              <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-md border border-border/60 bg-card text-foreground shadow-xs">
          ...
          <div className="flex flex-wrap gap-2 border-t border-border/60 bg-card px-4 py-3">
            {activeFilters.map((filter) => (
              <button
                ...
                className="inline-flex h-7 max-w-full items-center gap-1.5 rounded-md border border-border/60 bg-muted px-2.5 text-xs text-foreground transition-colors hover:border-border hover:bg-card focus-visible:border-ring focus-visible:ring-ring/50 focus-visible:ring-[3px] focus-visible:outline-none"
                aria-label={`Remove ${filter.label} filter`}
              >
                <span className="shrink-0 font-medium text-muted-foreground">{filter.label}</span>
                <span className="max-w-[16rem] truncate text-foreground">{filter.value}</span>
  ...
  return (
    <FilterField label={label} className={containerClassName}>
      <div className="relative">
        ...
        <Input className={cn('h-10 bg-card pl-10 text-sm', className)} {...props} />
      </div>
    </FilterField>
  );
}
